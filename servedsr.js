...
app.get('/api/chemical_2025', async (req, res) => {
  try {
    ...
    res.status(500).send('Error fetching Data'); // Send a more descriptive error message
  }
});
...
app.put('/employees/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const {
      emp_name, designation, email, mobile
    } = req.body;
    
    const [result] = await pool.query(
      `UPDATE employee SET emp_name = ?, designation = ? , email = ?, mobile = ? WHERE id = ?`,
      [
        emp_name, designation, email, mobile
      ]
    );
...
app.post('/api/employees', async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      `INSERT INTO employee (emp_name, designation, email, mobile) 
      VALUES (?, ?, ?, ?)`,
      [
        emp_name, designation,email,mobile
      ]
    ...
    const newEmployee = { emp_id: result.Id, ...req.body };
